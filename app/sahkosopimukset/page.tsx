...
/*
  SÄHKÖVERTAILU ASUU TÄÄLLÄ, EI ETUSIVULLA.

  Vertailu oli hetken aikaa etusivuna (`app/page.tsx`). Se palautettiin
  tähän 17.8.2026, kun etusivusta tehtiin oikea hub kahdelle palvelulle.

  MIKSI JUURI TÄMÄ OSOITE EIKÄ JOKIN UUSI: `/sahkosopimukset` on ollut
  Googlen indeksissä jo kerran, ja sen alla asuvat sekä sopimussivut
  (`/sahkosopimukset/sopimus/[slug]`) että aihesivut
  (`/sahkosopimukset/[topic]`). Kun vertailu oli etusivulla, noiden
  lasten emosivu oli pelkkä uudelleenohjaus. Nyt polku on kokonainen.

  OHJAUS ON POISTETTU `next.config.mjs`:STÄ. Se osoitti aiemmin
  `/sahkosopimukset` → `/`. Jos se palautetaan, tämä sivu ei aukea
  koskaan: palvelin ohjaa pois ennen kuin ehtii renderöidä. Älä
  palauta sitä riviä.

  PORTTI SÄILYY. Tuloslista on yhä kyselyn takana (`withHero` → `gated`
  komponentissa `ElectricityExperience`). Osoitteen vaihtuminen ei
  muuta sitä päätöstä millään tavalla.
*/
...
export const metadata: Metadata = {
  ...
  */
  ...
  /*
    KANONINEN OSOITE ON TÄMÄ SIVU, EI ENÄÄ `/`. Jos tämä jäisi
    osoittamaan etusivulle, kertoisimme Googlelle että sähkövertailun
    oikea koti on hub — ja hub ei sisällä yhtään sopimusta. Sivu
    poistuisi hausta omilla asiasanoillaan.
  */
...
export default function SahkosopimuksetPage() {
  const plans = getPlans();
  const topics = getEnergyTopics();

  /*
    ORGANIZATION-MERKINTÄ SIIRTYI ETUSIVULLE (`app/page.tsx`).

    Se kuvaa koko sivustoa, ei tätä sivua, ja `url`-kenttä osoittaa
    juureen. Alasivulla se olisi ristiriita: merkintä väittäisi tämän
    osoitteen olevan organisaation koti. Samasta syystä sitä ei saa
  ...
  */
  ...
  return (
    <>
      ...
      {/*
        TÄMÄ SIVU ON PORTIN TAKANA: sopimuksia ei näytetä ennen kuin kysely
      ...
      */}
      ...
      {/*
        PALVELUVALIKOIMA SIIRTYI ETUSIVULLE. Se kertoo mitä Kettu
        kilpailuttaa, ja se on nyt hubin tehtävä. Tällä sivulla se olisi
        ollut ainoa osio, joka linkittää pois sähkösuppilosta — eli
        pelkkä vuotokohta juuri siellä, missä koko sivuston tuotto
        syntyy. Kävijä on jo valinnut sähkön päätyessään tänne.
      */}
      ...
      */}
      ...
      {/* `href` jätetään pois tarkoituksella: `CtaSection`in oletus on
          `ENERGY_COMPARE` (lib/nav.ts). Kovakoodattu "/#vertailu" osoitti
          osoitteen vaihduttua hubin ankkuriin, jota ei ole olemassa. */}
      <CtaSection
