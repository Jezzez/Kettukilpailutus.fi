  ...
  return (
    <>
      ...
      <section className="den-surface relative overflow-hidden">
        <div className="relative mx-auto grid max-w-[1180px] items-center gap-6 px-4 pb-16 pt-14 sm:px-6 md:grid-cols-[1.08fr_0.92fr] md:pb-20 md:pt-20">
          <div>
            ...
            <h1 className="mt-4 font-display text-[2.7rem] font-extrabold leading-[1.03] tracking-tight text-cream sm:text-[3.5rem]">
              Yksi kettu.<br />Kaikki <span className="text-accent">kilpailutukset</span>.
            </h1>
            ...
            <div className="mt-8 flex flex-wrap gap-3">
              <Link
                href="/sahkosopimukset"
                className="group inline-flex items-center gap-2.5 btn-ember rounded-xl px-7 py-4 font-display text-[15.5px] font-bold text-cream transition-all active:scale-[0.98]"
              >
              ...
              <Link
                href="/luottokortit"
                className="inline-flex items-center gap-2.5 rounded-xl border border-cream/15 bg-cream/5 px-7 py-4 font-display text-[15.5px] font-bold text-cream backdrop-blur transition-all hover:bg-cream/10 active:scale-[0.98]"
              >
                <CreditCard size={18} aria-hidden /> Vertaa kortit
              </Link>
            </div>
          </div>
              ...
              <Reveal key={c.title} delay={i * 0.05}>
                {c.live ? (
                  ...
                  >
                    ...
                    <span className="grid h-12 w-12 place-items-center rounded-2xl border border-accent/25 bg-accentSoft text-accent transition-transform group-hover:scale-110">
                      <c.icon size={22} aria-hidden />
                    </span>
                    <h2 className="mt-4 font-display text-xl font-semibold text-ink">{c.title}</h2>
                    <p className="mt-1.5 flex-1 text-[14px] leading-relaxed text-ink/72">{c.desc}</p>
                  ...
                  <div className="flex h-full flex-col rounded-3xl border border-dashed border-line bg-white/50 p-7" aria-disabled>
                    ...
                    <h2 className="mt-4 font-display text-xl font-semibold text-ink/58">{c.title}</h2>
                    <p className="mt-1.5 flex-1 text-[14px] leading-relaxed text-ink/55">{c.desc}</p>
                    <span className="mt-4 inline-block w-fit rounded-full bg-mist px-3 py-1 text-[12px] font-semibold text-ink/58">
                      Tulossa pian
                    </span>
                  </div>
                )}
            ...
            {[
              ...
              <Reveal key={f.title} delay={i * 0.08}>
                <div className="rounded-2xl bg-mist p-6 text-center">
                  <span className="mx-auto grid h-12 w-12 place-items-center rounded-2xl bg-white text-accent">
                    <f.icon size={22} aria-hidden />
                  </span>
                  <h3 className="mt-4 font-display text-lg font-semibold text-ink">{f.title}</h3>
                  <p className="mt-2 text-[14px] leading-relaxed text-ink/72">{f.text}</p>
                </div>
              </Reveal>
            ))}
          </div>
