...
export default function ElectricityExperience({
  ...
  */
  ...
  /*
    VERTAILUPERUSTE ON ENSI KUUN HINTA. JÄRJESTYS ON ENSIMMÄINEN VUOSI.

    MIKSI JUURI NÄIN PÄIN, KAHDELLA ERI SYYLLÄ.

    Kävijä kirjoittaa laskustaan kaksi lukua: perusmaksun ja marginaalin,
    jotka ovat voimassa NYT. Se on hänen ainoa vertailukohtansa. Jos
    vastaamme siihen lukuun ensimmäisen vuoden keskiarvolla, vertaamme
    kahta eri aikaikkunaa keskenään, ja vastaus on väärä molempiin
    suuntiin. Konkreettinen tapaus: kävijä maksaa 0 €/kk ja 0,89 c/kWh,
    Cheap Energy antaa juuri nyt 0,39 c/kWh ilman perusmaksua, eli me
    olemme selvästi halvempia — ja sivu silti ilmoitti, ettei mitään
    halvempaa löydy, koska Cheapin vuosikeskiarvo sisälsi kampanjan
    jälkeisen hinnan. Se on pahin mahdollinen virhe: se hylkää oikean
    suosituksen ja menettää klikin, joka oli jo ansaittu.

    Sivu vastaa siis kysymykseen, jonka kävijä oikeasti esitti: mitä
    laskussani lukee ensi kuussa, ja onko teillä sitä halvempaa. Näkyvät
    kuukausihinnat, "Ketun valinta", säästöluku, hintapalkit ja "älä
    vaihda" -verdikti lasketaan kaikki tästä yhdestä luvusta, jolloin
    kummallakin puolella vertailua on sama aikaikkuna.

    JÄRJESTYS ON SILTI VUOSIHINNASSA, ks. `jarjestysHinta`. Kampanjahinta
    ei tiedä kampanjan pituudesta mitään, joten pelkällä sillä
    järjestettynä neljän kuukauden houkutin nousee vuoden kestävän
    alennuksen ohi: Aalto Luotsi oli 7 000 kWh:lla ykkönen hinnalla
    31,6 €/kk vaikka maksaa ensimmäisenä vuonna 504 € ja sen jälkeen
    47,2 €/kk, ja halvin sopimus oli kahdeksantena.

    TIETOINEN SEURAUS: lista ei ole ison luvun mukaisessa järjestyksessä,
    ja "Ketun valinta" voi osua muuhun kuin listan ensimmäiseen korttiin.
    Kortti selittää eron itse kahdella luvulla, "Sitten X €/kk" ja
    "Y € vuodessa". Näitä kahta riviä ei saa poistaa: ilman niitä
    järjestys näyttää satunnaiselta, ja satunnaiselta näyttävä
    hintajärjestys on nopein tapa menettää lukijan luottamus.
  */
  ...
  /** Listan järjestysperuste: ensimmäisen vuoden kokonaishinta kampanjoineen.
   *  Käytetään VAIN `filtered`-lajittelussa. Ks. `vertailuhinta` yllä. */
  const jarjestysHinta = useCallback(
  ...
  /*
    ...
    1. EHDOKKAAKSI PÄÄSEE VAIN SOPIMUS, JOKA VOITTAA ASIAKKAAN HINNAN
       JUURI NYT. Jos kävijä maksaa 0 € perusmaksua ja 0,89 c/kWh
       marginaalia, Kettu ei saa ehdottaa mitään, mikä on tänään
       kalliimpaa. Kävijä vertaa suositusta siihen lukuun, jonka hän
       juuri itse kirjoitti, ja jos suositus häviää sille, koko sivu
       menettää uskottavuutensa yhdessä sekunnissa.

    2. EHDOKKAISTA VALITAAN HALVIN ENSI KUUSSA. Sama mittari kuin
       korttien isot luvut ja säästöluku, joten paneeli ja kortti eivät
       voi näyttää eri hintaa samasta sopimuksesta.

    Painokertoimia ei ole. Molemmat säännöt voi tarkistaa korttien omista
    luvuista, ja se on tämän sivun ainoa kilpailuetu.

    Jos kävijä ei ole kertonut omaa hintaansa, sääntö 1 ei voi rajata
    mitään, ja tassu menee halvimpaan ensi kuun hintaan.

    HUOM: lista järjestetään vuosihinnalla (`jarjestysHinta`), joten
    tassu ei ole aina listan ensimmäisessä kortissa. Se on tietoinen
    valinta, ks. `vertailuhinta`-kommentti.
  */
      ...
      plans
        ...
        .sort((a, b) => {
          ...
          if (sort === "campaign") {
            ...
            if (d !== 0) return d;
            return jarjestysHinta(a) - jarjestysHinta(b);
          }
          return jarjestysHinta(a) - jarjestysHinta(b);
        }),
    [plans, type, greenOnly, kwh, sort, jarjestysHinta]
  );
  ...
  /** Koko aineiston halvin ensi kuun hinnalla, suodattimista riippumatta.
   *  Nimetty luku "älä vaihda" -laatikkoon: nimetön vertailuluku on
   *  tarkistamaton väite, nimetty on listasta tarkistettavissa sekunnissa. */
  const cheapestNowPlan = useMemo(() => {
    if (plans.length === 0) return null;
    return [...plans].sort((a, b) => vertailuhinta(a) - vertailuhinta(b))[0];
  }, [plans, vertailuhinta]);
   ...
   *
   *  `cheapestCost` (koko aineiston halvin) jää erikseen: "älä vaihda"
   *  saa sanoa vain, jos asiakkaan sopimus voittaa KAIKKI sopimukset,
   *  ei vain sattumalta valittua tyyppiä.
   *
   *  Tästä poistettiin `bestVisibleCost` ja `headlinePlan`. Molemmat
  ...
  const cheapestPlan = useMemo(() => {
    ...
    /* Sääntö 1 — ehdokkaaksi pääsee vain sopimus, joka on asiakkaan
       nykyistä halvempi ENSI KUUSSA. Molemmat luvut ovat siis samasta
       aikaikkunasta: hänen laskunsa nyt ja sopimuksen hinta nyt. */
    const ehdokkaat =
      currentAnnual === null
        ? filtered
        : filtered.filter((p) => vertailuhinta(p) < currentAnnual);

    /* Sääntö 2 — ehdokkaista se, jonka ENSIMMÄINEN VUOSI on halvin.
       Tämä on ainoa kohta, jossa suositus ei katso ensi kuuta.
       Syy: sääntö 1 takaa jo, ettei ehdokas ole nykyistä kalliimpi ensi
       kuussa, joten ensi kuun hinnalla valitseminen ei enää suojaa
       mitään — se vain nostaisi esiin lyhimmän kampanjan. Käytännössä
       se valitsisi Aalto Luotsin (31,62 €/kk nyt, 47,19 €/kk viidennestä
       kuukaudesta), vaikka listan ykkönen Nordic Green Ilmasto maksaa
       ensimmäisenä vuonna 89 € vähemmän. Syöttihinnan suosittelu on
       tälle sivustolle kalliimpi virhe kuin muutaman euron ero
       otsikkoluvussa: palkkio maksetaan sopimuksesta, ja peruttu
       sopimus vie palkkion mukanaan.

       Järjestys tehdään uudelleen näkyviin, jotta sääntö ei ole
       riippuvainen siitä mitä ylempänä sattuu tapahtumaan. */
    if (ehdokkaat.length > 0) {
      return [...ehdokkaat].sort((a, b) => jarjestysHinta(a) - jarjestysHinta(b))[0];
    }

    /* Yksikään tämän tyypin sopimus ei voita asiakkaan hintaa. `alreadyGood`
       katsoo koko aineistoa ja tämä lista on tyypillä rajattu, joten tähän
       päädytään myös silloin kun jokin toisen tyypin sopimus olisi
       halvempi. Fallbackina listan halvin ensimmäinen vuosi, samalla
       säännöllä kuin yllä. */
    return [...filtered].sort((a, b) => jarjestysHinta(a) - jarjestysHinta(b))[0];
  }, [filtered, currentAnnual, vertailuhinta, jarjestysHinta]);
  ...
  */
  ...
  /*
    ...
    SUOSITUS ON AINA HALVIN ENSI KUUSSA — EI PAINOTETTU VALINTA.

    Tässä suositeltiin aiemmin Ketun valintaa (ensimmäinen vuosi 50 % ·
    kampanjan jälkeinen hinta 50 %). Se tarkoitti, että sivun ylälaidassa
    oli kaksi laatikkoa peräkkäin: toinen nimesi halvimman, toinen
    suositteli usein jotakin muuta. Kävijä joutui lukemaan perustelun
    ymmärtääkseen, miksi hänelle suositellaan kalliimpaa — ja jokainen
    tällainen sekunti on pois "Tee sopimus" -napista. Pahempaa: se on
    juuri se kuvio, jonka epäluuloinen lukija tulkitsee palkkion
    ohjaamaksi, ja epäilty vertailu menettää klikin lisäksi paluukäynnin.

    Nyt vastaus on yksi ja tarkistettavissa sekunnissa: Kettu suosittelee
    sitä sopimusta, joka on hänen kulutuksellaan halvin. Väite, jonka voi
    tarkistaa listan ensimmäisestä kortista, ei vaadi luottamusta
    lainkaan — ja se on tällä sivustolla arvokkaampaa kuin hienompi kaava.

    Sama sopimus saa korteissa kultaisen "Ketun valinta" -merkin, ja
    mobiilin tulospalkki vie samaan. Koko sivu antaa siis yhden vastauksen.
  */
  const recommendedPlan = cheapestPlan;

  /*
    LISTA ON PUHTAASSA HINTAJÄRJESTYKSESSÄ — EI NOSTOJA.

    Ketun valinta nostettiin ennen aina toiseksi kortiksi, koska se
    laskettiin eri kaavalla kuin järjestys ja saattoi muuten pudota
    kohtaan, jota mobiilikävijä ei näe koskaan. Nyt merkki on halvimmalla
    eli listan ensimmäisellä kortilla, joten nostettavaa ei ole.

    Se on myös parempi näin: siirretty kortti on aina selittämätön hyppy
    järjestyksessä, ja järjestys on ainoa asia, jonka kävijä voi tarkistaa
    korttien omista luvuista. Kun mikään ei ole siirretty, sen voi tarkistaa
    ilman selitystä.
  */
  /*
  ...
  */
  const recommendedReasons = recommendedPlan
    ? [
        /* Perustelun on kuvattava TÄSMÄLLEEN se laskutoimitus, joka
           ...
           tämä siis välttämättä ole koko listan halvin.
           Sen sanominen ääneen on halvempaa kuin se, että lukija laskee
           korteista perässä ja huomaa väitteen ontuvan. */
        currentAnnual === null
          ? "Halvin ensimmäinen vuosi"
          : "Halvin vuosi niistä, jotka voittavat hintasi jo ensi kuussa",
        ...
        /* Neljäs rivi oli asiakasarvio. Arvioita ei ole enää olemassa,
           joten tilalla on tieto, joka on: joko kampanjaetu euroina tai
           — jos kampanjaa ei ole — se että hinta ei nouse vuoden
           jälkeen. Kampanjahinnan takaa paljastuva korkeampi normaali-
           hinta on tämän alan yleisin pettymys, joten se kerrotaan tässä
           eikä vasta laskussa. */
  ...
  /** Rehellisyys ennen konversiota: jos asiakkaan oma sopimus voittaa, se
   *  sanotaan. Molemmat luvut ovat ensi kuun hintoja, joten vertailu on
   *  aidosti vertailukelpoinen. Tässä oli aiemmin turvaraja
   *  kampanjasyötteitä varten; sitä ei enää tarvita, koska epäsymmetria
   *  jonka takia se rakennettiin poistui aikaikkunan vaihdon myötä. */
  ...
  /*
    LUKU JUOKSEE PAIKALLEEN, EI ILMESTY VALMIINA.

    Säästöluku syntyy vasta kun kävijä on itse syöttänyt nykyisen
    hintansa. Jos se vain pomppaa ruudulle valmiina, se näyttää samalta
    kuin mikä tahansa sivulla valmiiksi ollut mainosluku. Nouseva luku
    näyttää siltä, että se laskettiin juuri hänen antamastaan tiedosta —
    ja se on myös totta. Sama `useCountUp` kuin muuallakin, eli
    liikeherkkyysasetus ja piilotettu välilehti hoituvat automaattisesti.
  */
  const savingCounted = useCountUp(savingVsCurrent);
    ...
    <div className="grid gap-2.5">
      {PREFS.map((o) => (
        <BigOption
          ...
          onClick={() => {
            setPricePref(o.key);
            if (gated && !submitted && step === 3) {
              if (advanceTimer.current) clearTimeout(advanceTimer.current);
              advanceTimer.current = setTimeout(() => setStep((cur) => (cur === 3 ? 4 : cur)), 260);
            }
          }}
        />
      ))}
  ...
  */
  ...
  const loadingPanel = (
    <div className="overflow-hidden rounded-2xl border border-gold/30 bg-mist sm:rounded-[20px]">
      <div className="grid items-stretch md:grid-cols-[0.72fr_1.28fr]">
        <div className="pelt-surface relative flex h-44 items-center justify-center overflow-hidden border-b border-gold/25 md:h-auto md:min-h-[320px] md:border-b-0 md:border-r">
          <FoxSlot
            id="laskuri"
            height={330}
            className="pointer-events-none -mb-20 -mt-8 drop-shadow-[0_12px_24px_rgba(90,45,10,0.16)] md:absolute md:inset-y-0 md:left-1/2 md:mb-0 md:mt-0 md:!h-full md:-translate-x-1/2"
          />
        </div>

        <div className="flex flex-col justify-center p-5 text-center sm:p-7 md:min-h-[320px] md:p-9 md:text-left" role="status" aria-live="polite">
          <p className="flex items-center justify-center gap-2.5 font-display text-[11.5px] font-bold uppercase tracking-[0.18em] text-accentDark md:justify-start">
          ...
          <div className="mt-4 h-1.5 overflow-hidden rounded-full bg-line md:mt-5" aria-hidden>
            <motion.div
              ...
              initial={{ width: "0%" }}
              animate={{ width: "100%" }}
              transition={{ duration: RESULT_LOADING_MS / 1000, ease: "linear" }}
            />
          </div>
        </div>
      </div>
  ...
  const estimator = (dark: boolean) => (
    ...
    >
      {loadingResults ? loadingPanel : gated && !submitted ? wizard : fullForm}
      {!loadingResults && trustStrip}
    </div>
  );
      ...
      */}
      ...
      <section ref={resultsTopRef} className={`theme-light scroll-mt-20 bg-paper ${withHero ? "pt-14" : "pt-10"}`}>
        <div className="mx-auto max-w-[1180px] px-4 sm:px-6">
          {/*
            "ÄLÄ VAIHDA" — SIVUN AINOA JÄLJELLE JÄÄVÄ YLÄLAATIKKO.

            Tämän yläpuolella oli aiemmin "Ketun löytö sinun luvuillasi"
            -laatikko, joka nimesi halvimman sopimuksen omalla napillaan.
            Se poistettiin, koska Ketun suositus on nyt sama sopimus: kaksi
            laatikkoa peräkkäin sanoi saman asian kahteen kertaan, työnsi
            suosituksen alemmas ruudulla ja pakotti kävijän valitsemaan
            kahdesta napista, jotka veivät samaan paikkaan. Kaksi
            samannäköistä kehotusta ei tuota kahta klikkiä vaan yhden
            epäröinnin.

            TÄMÄ TILA EI KUITENKAAN SAA KADOTA MUKANA. Kun asiakkaan oma
            sopimus on jo halvempi, palvelu sanoo sen suoraan ja luopuu
            omasta palkkiostaan — se on sivun tärkein luottamussignaali ja
            ainoa hetki, jossa kävijä oikeasti uskoo kaiken muunkin.
            Suosituspaneeli on tässä tilassa piilotettu, joten ilman tätä
            palikkaa ruudulla ei lukisi yhtään mitään.

            Kettu tulee mukaan kämmen ylöspäin: kuva tekee kieltäytymisestä
            luonteenpiirteen eikä pelkän ilmoituksen, ja juuri sen takia
            kävijä palaa ensi vuonna takaisin.

            UMPIKUJA KORJATTIIN, VERDIKTIÄ EI MUUTETTU. Laatikossa luki
            aiemmin pelkkä "Älä vaihda nyt", eikä sen jälkeen ollut mitään
            tehtävää: kävijä oli antanut kulutuksensa ja hintansa ja sai
            vastauksena kiellon. Rehellinen kielto on yhä oikea vastaus,
            mutta se ei saa olla koko vastaus. Nyt laatikko näyttää myös
            luvut joihin verdikti perustuu (oma hinta ja nimetty halvin
            vertailussa), ja antaa sen ainoan tehtävän joka tässä tilassa
            on aidosti hyödyllinen: tarkista oman sopimuksesi päättymispäivä.

            Miksi juuri se: tähän tilaan päädytään lähes aina hyvän
            määräaikaisen tai kampanjahinnan takia, ja molemmat päättyvät.
            Kävijä, joka poistuu tietäen milloin hänen kannattaa palata,
            on tuoton kannalta arvokkaampi kuin kävijä joka painostettiin
            vaihtamaan huonompaan — jälkimmäinen on peruutus ja menetetty
            luottamus, edellinen paluukäynti.

            Ostokehotusta tähän EI lisätä. Suosituspaneeli pysyy piilossa
            tässä tilassa: "älä vaihda" ja oranssi "Tee sopimus" vierekkäin
            mitätöisivät toisensa, ja koko laatikon arvo on siinä, että se
            luopuu palkkiosta näkyvästi. Vertailulista on silti alla
            kokonaisuudessaan, joten kävijä pääsee itse katsomaan luvut.
          */}
          {alreadyGood && currentAnnual !== null && (
            <div className="flex w-full max-w-2xl items-center gap-5 rounded-2xl border border-gold/35 bg-white px-6 py-5 shadow-card sm:rounded-[20px]">
              ...
              <div>
                ...
                {/* Väite on tarkka: laatikko näkyy vain kun asiakkaan oma
                    ensi kuun hinta voittaa jokaisen vertailun sopimuksen
                    ensi kuun hinnan. Sama mittari kuin korteissa. */}
                <p className="mt-2 font-display text-[16px] font-bold leading-snug text-ink">
                  Maksat jo vähemmän kuin yksikään vertailun sopimus.
                </p>

                {/* Verdikti ilman lukuja on pyyntö uskoa. Nimetty luku on
                    listasta tarkistettavissa sekunnissa, ja juuri se
                    tarkistettavuus on syy uskoa myös se kohta, jossa
                    palvelu luopuu palkkiosta. */}
                {cheapestNowPlan && (
                  <div className="mt-3 flex flex-wrap items-baseline gap-x-3 gap-y-0.5 rounded-xl bg-mist px-3 py-2">
                    <span className="font-data text-[15.5px] font-bold text-ink">
                      sinä{" "}
                      {(currentAnnual / 12).toLocaleString("fi-FI", {
                    ...
                    <span className="font-data text-[15.5px] font-bold text-ink/70">
                      halvin{" "}
                      {(vertailuhinta(cheapestNowPlan) / 12).toLocaleString("fi-FI", {
                        minimumFractionDigits: 2,
                        maximumFractionDigits: 2,
                      })}{" "}
                      €
                    ...
                    <span className="font-display text-[12px] font-semibold text-ink/50">
                      {cheapestNowPlan.provider} {cheapestNowPlan.name}
                    </span>
                  </div>
                )}

                <p className="mt-3 text-[13px] leading-relaxed text-ink/70">
                  Vaihtaminen ei kannata nyt. Tarkista sopimuksestasi, milloin
                  hintasi nousee, ja palaa silloin.
                </p>
              </div>
            </div>
          )}
          ...
          {/*
            ...
            MIKSI SE ON REHELLINEN: valintaperuste on yksi lause —
            halvin vuosihinta kävijän omalla kulutuksella — ja sen voi
            tarkistaa listan ensimmäisestä kortista sekunnissa. Sama
            sopimus kantaa korteissa "Ketun valinta" -merkin, joten
            paneeli, merkki ja järjestys eivät voi riidellä keskenään.
            Jos asiakkaan oma sopimus on jo halvempi, tätä paneelia ei
          ...
          */}
          {recommendedPlan && !alreadyGood && (
            <motion.div
              ...
              className="theme-ember ember-surface relative overflow-hidden rounded-3xl border border-line shadow-lift"
            >
                ...
                <div className="min-w-0 flex-1">
                  ...
                  */}
                  ...
                  <div className="mt-5 flex flex-wrap items-center gap-x-7 gap-y-3">
                    <div>
                      {/* ISO LUKU ON SAMA, JOLLA VALINTA TEHTIIN, ELI ENSI
                          KUUN HINTA. Jos paneelin näkyvin luku olisi eri
                          kuin se, jolla suositus laskettiin, lukija ei
                          voisi tarkistaa kumpaakaan. */}
                    ...
                    <div>
                      {/* Iso luku on kampanjahinta, eli se loppuu. Tämä
                          rivi kertoo mihin se nousee: ilman sitä paneeli
                          lupaisi hinnan, joka nousee ilman varoitusta, ja
                          se nousu on tämän alan yleisin pettymys. */}
                      <p className="text-[12px] font-semibold uppercase tracking-[0.14em] text-goldInk">
                        {recommendedPlan.campaign ? "Sitten" : "Vuodessa"}
                      </p>
                    ...
                    {/*
                      SÄÄSTÖLUKU NÄKYY VAIN KUN SE ON TOSI.

                      Kolmas paikka jää tyhjäksi, jos kävijä ei ole kertonut
                      nykyistä hintaansa. Tässä oli hetken kehotuslaatikko
                      ("Säästösi euroina" + nappi), mutta se ei jää: paneelin
                      tehtävä on esittää yksi vastaus ja yksi nappi. Toinen,
                      kilpaileva nappi aivan "Tee sopimus" -napin vieressä
                      jakaa katseen juuri siinä kohdassa, jossa tuotto
                      syntyy — ja se toinen nappi vie pois päätöksestä
                      takaisin lomakkeeseen.

                      SYÖTTÖ EI KATOA MINNEKÄÄN: nykyisen hinnan kenttä on
                      laskurissa ("Nykyinen sopimuksesi"), eli se kävijä joka
                      haluaa oman säästölukunsa löytää sen yhä. Kun hinta on
                      annettu, luku liukuu tähän paikkaan.

                      VAIHTO ON PEHMEÄ, EI HYPPY. `AnimatePresence` +
                      `popLayout`: luku liukuu paikalleen sen sijaan että
                      paneeli nytkähtäisi uuteen korkeuteen. Nytkähdys juuri
                      ostonapin vieressä lukee virheeksi, eikä virheeltä
                      näyttävää nappia paineta.
                    */}
                    <AnimatePresence mode="popLayout" initial={false}>
                      {currentAnnual !== null && currentAnnual > recommendedAnnual ? (
                        <motion.div
                          key="saasto"
                        ...
                        >
                          {/* Säästö on ero KUUKAUDESSA, koska molemmat luvut
                              joista se lasketaan ovat ensi kuun hintoja.
                              Vuosiluku olisi tässä ekstrapolaatio: se
                              olettaisi kampanjan jatkuvan koko vuoden, eikä
                              se jatku. Kampanjan jälkeinen hinta lukee
                              vieressä, joten lukija näkee itse mihin asti
                              tämä säästö on voimassa. */}
                          <p className="text-[12px] font-semibold uppercase tracking-[0.14em] text-goldInk">
                            Säästö nykyiseesi
                          </p>
                          <p className="mt-0.5 font-display font-data text-[1.35rem] font-bold leading-none text-cream">
          ...
          */}
          ...
          >
            ...
            <label className="flex items-center gap-2 text-[13px] text-ink/70">
              ...
              >
                <option value="cost">Halvin 1. vuosi</option>
            ...
            <motion.div layout={!reduce} className="mt-4 grid grid-cols-1 gap-4 pt-2 sm:grid-cols-2 lg:grid-cols-3">
              <AnimatePresence mode="popLayout">
                {(naytaKaikki ? naytettavat : naytettavat.slice(0, NAYTA_ALUKSI)).map((plan, i) => {
                  ...
                  const numero = i + 1;
                  /*
                    YKSI MERKKI, YKSI KORTTI: HALVIN SAA KETUN VALINNAN.

                    Merkkejä oli kaksi: kerma "Edullisin tällä hetkellä" ja
                    kulta "Ketun valinta", ja ne osuivat usein eri kortteihin.
                    Kun suositus on nyt aina halvin, kaksi merkkiä olisi
                    antanut samalle kysymykselle kaksi vastausta samalla
                    ruudulla — ja kultainen sinetti eri kortissa kuin
                    suosituspaneelin nimi luetaan ostetuksi nostoksi. Se on
                    tämän sivuston kallein yksittäinen epäilys.

                    Kulta jäi, kerma lähti, koska sinetti on se elementti,
                    joka erottaa Ketun taulukosta. Väite pysyy silti
                    tarkistettavana: merkin alarivi sanoo, että kyse on
                    halvimmasta kävijän omalla kulutuksella, eli sen voi
                    todeta kortin omista luvuista sekunnissa.
                  */
                  const badge: PlanBadge = isCheapest ? { kind: "fox" } : null;
                    ...
                    >
                      <PlanCard
                        ...
                        /* Ero lasketaan SAMALLA perusteella kuin kortin iso
                           luku, eli ensi kuun hinnasta. Jos nämä kaksi
                           eroaisivat, samassa kortissa lukisi kaksi lukua,
                           jotka eivät voi molemmat olla totta, ja ristiriita
                           kortin sisällä on nopein tapa menettää lukijan
                           luottamus. Siksi tämä rivi ja `vertailuhinta` on
                           muutettava aina yhdessä. */
                        compareDiff={
