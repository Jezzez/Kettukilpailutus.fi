      ...
      <div className="mx-auto max-w-[1180px] px-4 sm:px-6">
        <div className="overflow-hidden rounded-3xl border border-line bg-white shadow-card">
          <div className="grid gap-px bg-line md:grid-cols-3">
            {STEPS.map((s, i) => (
              <div key={s.title} className="h-full bg-white p-7 text-center sm:p-8">
                <Reveal delay={i * 0.08}>
                  ...
                  <h3 className="mt-5 font-display text-[22px] font-semibold text-ink">{s.title}</h3>
                  <p className="mx-auto mt-2 max-w-xs text-[15px] leading-relaxed text-ink/75">{s.text}</p>
                </Reveal>
              </div>
            ))}
          </div>
